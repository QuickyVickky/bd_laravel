          ...
          <div class="row">
            ...
            <div class="col-sm-12">
              <div class="form-group required">
                <label>Name</label>
                <textarea type="text" rows="3" name="subscription_feature" class="form-control" id="subscription_feature" placeholder="Enter Subscription Feature" value="" required></textarea>
              </div>
            </div>
            <div class="col-sm-12">
              <div class="form-group required">
...
$.fn.dataTable.ext.errMode = 'none';
	...
	/*-------goods type---------*/
                    t1 = $('#t1').DataTable({
                        ...
                        serverSide: true,
                        ajax: {
						"url": "{{ route('get-subscription-feature-list') }}",
						"type": "get",
						"data":function(data) {
							data.onlytest =1;
						},
...
$('body').on('click', '#addidbtn', function () {
  ...
  $('body').on('click', '#btn_submit', function () {
        var id = $('#hid').val();
		var subscription_feature = $('textarea#subscription_feature').val();
		var status = $('input[name=status]:checked').val();

        if(id=='' || subscription_feature=='' || status==''){
     ...
     $.ajax({
            url: "{{ route('add-update-subscription-feature') }}",
            data: {
              _token:'{{ csrf_token() }}',
              hid: id, 
  ...
  $('body').on('click', '.edititmain', function () {
     ...
     $.ajax({
            url: "{{ route('edit-subscription-feature-data') }}",
            data: {
              _token:'{{ csrf_token() }}',
              id: id
            },
            ...
            success: function (data) {
              ...
              $('#subscription_feature').val(data.subscription_feature);
			  $('#hid').val(data.id);
              $("#addModal").modal('show');
            },
			error: function(jqXHR, textStatus, errorThrown) {
