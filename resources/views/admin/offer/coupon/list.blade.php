              ...
              <div class="row">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                  ...
                  <div class="alert alert-{{ Session::get('cls') }} mb-4" role="alert">
                    ...
                    {{ Session::get("msg") }}</div>
                  @endif </div>
                <div class="col-xl-6 col-md-6 col-sm-6 col-6"> <a href="{{ route('coupon-add') }}" class="btn btn-info m-2 btn-rounded float-right"> Add A New Coupon</a> </div>
              </div>
            </div>
...
$('body').on('click', '.editit', function () {
    ...
    $('body').on('click', '.change_status_confirm', function() {
        ...
        if (status == 1) {
            texttext = "Do You Want to DeActivate This Coupon ?";
        } else if (status == 0) {
            texttext = "Do You Want to Activate This Coupon ?";
        } else {
            return false;
        }
        ...
        swal({
            ...
            if (result.value) {
                ...
                $.ajax({
                    url: "{{ route('change-coupon-status') }}",
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id,
                        status: status,
