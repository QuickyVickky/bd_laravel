    ...
    <ul class="list-unstyled menu-categories" id="accordionExample">
      ...
      <?php
		...
		foreach($getAllMenus as $submenu){
			if(($submenu->main_url!='' && $submenu->main_url==Route::currentRouteName() && $ifouccurs==0 && $submenu->path_to==$menu->id) || ( $submenu->main_url!='' && in_array(Route::currentRouteName(), explode(',',$submenu->one_url)) && $ifouccurs==0 && $submenu->path_to==$menu->id ) ){
				$ifouccurs++;
			}
		}
		?>
      <li class="menu"> <a href="#sidebarmenu{{ $menu->id }}" data-toggle="collapse" data-active="{{ ($ifouccurs>0) ? 'true' : 'false' }}" aria-expanded="false" class="dropdown-toggle">
        ...
        <ul class="collapse submenu list-unstyled  {{ ($ifouccurs>0) ? 'show' : '' }}" id="sidebarmenu{{ $menu->id }}" data-parent="#accordionExample">
          @foreach($getAllMenus as $submenu)
          ...
          <li class="{{ (request()->is(env("ADMINBASE_NAME").'/'.$submenu->main_url) || in_array(Route::currentRouteName(), explode(',',$submenu->one_url))  ) ? 'active' : '' }}"> <a href="{{ route($submenu->main_url) }}" > {{ $submenu->name }} </a> </li>
          @endif
          @endif
          @endforeach
        </ul>
